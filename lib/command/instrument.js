...
function processFiles(instrumenter, coffee_instrumenter, inputDir, outputDir, relativeNames) {
    var processor = function (name, callback) {
            ...
            if (path.extname(inputFile) === '.js') {
                ...
                fs.readFile(inputFile, 'utf8', function (err, data) {
                    if (err) { return callback(err, name); }
                    coffee_instrumenter.instrument(data, inputFile, function (iErr, instrumented) {
                        if (iErr) { return callback(iErr, name); }
                        fs.writeFile(outputFile, instrumented, 'utf8', function (err) {
                            return callback(err, name);
                        });
                ...
                // non JavaScript/CoffeeScript file, copy it as is
...
Command.mix(InstrumentCommand, {
    ...
    run: function (args, callback) {
        ...
        if (needBaseline) {
            ...
            coffee_instrumenter = new BaselineCollector(coffee_instrumenter);
        ...
        if (stats.isDirectory()) {
            ...
            filesFor({
                ...
                processFiles(instrumenter, coffee_instrumenter, file, output, files);
            });
        } else {
            if (output) {
            ...
            if (path.extname(file) === '.coffee') {
                stream.write(coffee_instrumenter.instrumentSync(fs.readFileSync(file, 'utf8'), file));
            }
            else {
