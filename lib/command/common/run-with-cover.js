...
var Module = require('module'),
    ...
    CoffeeInstrumenter = require('../../coffee-instrumenter'),
...
function usage(arg0, command) {
    ...
    console.error('\nUsage: ' + arg0 + ' ' + command + ' [<options>] <executable-js-file-or-command> [-- <arguments-to-jsfile>]\n\nOptions are:\n\n'
        + [
            ...
            formatOption('  ', 'instrument all unused sources after running tests, defaults to false')
        ].join('\n\n') + '\n');
    console.error('\n');
}

function run(args, commandName, enableHooks, callback) {
    if (verbose) { console.error('initial values', args, commandName, enableHooks, callback); }
    ...
    runFn = function () {
        ...
        if (verbose) { console.log('Running: ' + process.argv.join(' ')); }
    ...
    if (enableHooks) {
        if (verbose) { console.error('hooks are enabled'); }
        ...
        if (verbose) { console.error('excludes: ' + excludes); }
        if (verbose) { console.error('root: ' + config.instrumentation.root() || process.cwd()); }
        ...
        matcherFor({
            ...
            function (err, matchFn) {
                ...
                var coverageVar = '$$cov_' + new Date().getTime() + '$$',
                    ...
                    coffee_instrumenter = new CoffeeInstrumenter({
                        coverageVariable: coverageVar ,
                        preserveComments: preserveComments
                    }),
                    transformer,
                ...
                transformer = function (code, filename) {
                    if (verbose) { console.error('instrumenting ' + filename); }

                    if (path.extname(filename) === '.js') {
                        return instrumenter.instrumentSync(code, filename);
                    } else {
                        return coffee_instrumenter.instrumentSync(code, filename);
                    }
                };
                ...
                if (config.hooks.hookRunInContext()) {
                    if (verbose) { console.error('hookRunInContext'); }
                ...
                process.once('exit', function () {
                    ...
                    if (config.instrumentation.includeAllSources()) {
                        ...
                        matchFn.files.forEach(function (file) {
                            if (!cov[file]) {
                                if (verbose) { console.error('covering source: ', file); }
                                transformer(fs.readFileSync(file, 'utf-8'), file);

                                var coverSources = function(menter) {
                                    Object.keys(menter.coverState.s).forEach(function (key) {
                                        menter.coverState.s[key] = 0;
                                    });

                                    cov[file] = menter.coverState;
                                }
                                ...
                                if (path.extname(file) === '.js') {
                                    coverSources(instrumenter);
                                } else {
                                    coverSources(coffee_instrumenter);
                                }
